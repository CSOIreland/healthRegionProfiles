...
SAWHRForJoinWithPX <- SAWHR%>%select(SA,Region)
SAWHR$AC <- "State"
EDWGUIDAC <- SAWHR
...
EDWGUIDAC$EDLC <- make.names(tolower(EDWGUIDAC$Region))
...
for (i in 1:nrow(EDWGUIDAC)) {
  ...
  tryCatch({
    ...
    ED <- as.character(EDWGUIDAC$Region[i])
    #EDGUID <- as.character(EDWGUIDAC$GUID[i])
    #EDGUIDLC <- as.character(EDWGUIDAC$GUIDLC[i])
    EDLC <- as.character(EDWGUIDAC$EDLC[i])
    if (ED!="State") {
    
    IHAsInHealthRegion <- as.data.frame(SAWHR$IHA[SAWHR$Region == ED])
    colnames(IHAsInHealthRegion)[1] <- "IHA"
    IHAsInHealthRegion <- IHAsInHealthRegion[!duplicated(IHAsInHealthRegion$IHA),]
    IHAsInHealthRegion <- as.data.frame(IHAsInHealthRegion)
    colnames(IHAsInHealthRegion)[1] <- "IHA"
    IHAsInHealthRegion <- IHAsInHealthRegion[order(IHAsInHealthRegion$IHA),]
    
    CHNsInHealthRegion <-  as.data.frame(SAWHR$CHN[SAWHR$Region == ED])
    colnames(CHNsInHealthRegion)[1] <- "CHN"
    CHNsInHealthRegion <- CHNsInHealthRegion[!duplicated(CHNsInHealthRegion$CHN),]
    CHNsInHealthRegion <- as.data.frame(CHNsInHealthRegion)
    colnames(CHNsInHealthRegion)[1] <- "CHN"
    CHNsInHealthRegion$CHN <- CHNsInHealthRegion[order(CHNsInHealthRegion$CHN),]
    
    IHAsInHealthRegionKable <- kable(IHAsInHealthRegion)
    #AC <- EDWGUIDAC$AC[i]
    AC<-"State"
   # ACGUID <- ACList$GUID[ACList$Administrative.Counties.2019 == AC]
    ...
    TitleForRMD <- paste0("Health Region Health Profile - ", ED)
