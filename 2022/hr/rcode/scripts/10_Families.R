FamSource <- as.data.frame(FamSource)
#ED table
...
FamAll <- bind_rows(FamED,FamAC)
FamAll$Region <- factor(FamAll$Region, levels = c(ED,AC))
...
FamPlot2 <- highchart()|>       
  hc_add_series(FamAll, "column", hcaes(x = Type.of.Family, y = PercentageOfFamilies, color = colouract,  group = Region), color = c('#405381','#FCBE72'), showInLegend = T)|>
  hc_yAxis(
