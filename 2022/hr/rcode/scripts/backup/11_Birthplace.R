BirthSource <- as.data.frame(BirthSource)
...
BirthLessTAll <- bind_rows(BirthEDLessT,BirthACLessT)
BirthLessTAll$Region <- factor(BirthLessTAll$Region, levels = c(ED,AC))
#plot
...
BirthPlot2 <- highchart()|>       
  hc_add_series(BirthLessTAll, "column", hcaes(x = Location2, y = PercentageOfPopulation, color = colouract,  group = Region), color = c('#405381','#FCBE72'), showInLegend = T)|>
  hc_yAxis(
