SocialClassSourceTable <- as.data.frame(SocialClassSourceTable)
# ED Table
...
SocialClassBinded <- bind_rows(SocialClassED,SocialClassAC)%>%filter(Social.Class!="Total")
SocialClassBinded$Region <- factor(SocialClassBinded$Region, levels = c(ED,AC))
...
SocialClassPlot2 <- highchart()|>       
  hc_add_series(SocialClassBinded, "column", hcaes(x = Social.Class2, y = PercentageOfPopulation, color = colouract,  group = Region), color = c('#405381','#FCBE72'), showInLegend = T)|>
  hc_yAxis(
