PESSource <- as.data.frame(PESSource)
...
PESLessTAll <- bind_rows(PESEDLessT,PESACLessT)
PESLessTAll$Region <- factor(PESLessTAll$Region, levels = c(ED,AC))
...
PESPlot2 <- highchart()|>       
  hc_add_series(PESLessTAll, "column", hcaes(x = Principle.Economic.Status2, y = PercentageOfPopulation, color = colouract,  group = Region), color = c('#405381','#FCBE72'), showInLegend = T)|>
  hc_yAxis(
