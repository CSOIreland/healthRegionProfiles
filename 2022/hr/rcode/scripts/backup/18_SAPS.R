SAPSPercentagesEDACState <- SAPSPercentages[SAPSPercentages$Region == ED | 	SAPSPercentages$Region == AC,]
SAPSPercentagesEDACState <- SAPSPercentagesEDACState[!duplicated(SAPSPercentagesEDACState$Region),]
# Add region var
#SAPSPercentagesEDACState$Region[SAPSPercentagesEDACState$GUID == "IE0"] <- "State"
SAPSPercentagesEDACState$Region[SAPSPercentagesEDACState$Region == ED] <- "ED"
SAPSPercentagesEDACState$Region[SAPSPercentagesEDACState$Region == AC] <- "State"

# Transpose the dataframe so tables can be more easily created
SAPSTransposed <- as.data.frame(t(SAPSPercentagesEDACState))
colnames(SAPSTransposed) <- SAPSTransposed[rownames(SAPSTransposed) == "Region",]
SAPSTransposed <- SAPSTransposed[rownames(SAPSTransposed)!="Region", ]
...
ColsToSelect <- c("Label", "ED", "State", "LatexTableTitle","Rank")
