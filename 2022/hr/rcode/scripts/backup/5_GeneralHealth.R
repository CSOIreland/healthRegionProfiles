GenWider <- as.data.frame(GenWider)
#ED subtables
...
GenLongBinded <- bind_rows(GenLongEDBothSexesGraph,GenLongACBothSexesGraph)
GenLongBinded$Region <- factor(GenLongBinded$Region, levels = c(ED,AC))
#plot general health
...
GenPlot2 <- highchart()|>       
  hc_add_series(GenLongBinded, "column", hcaes(x = General.Health, y = Percentage.Of.Population, color = colouract,  group = Region), color = c('#405381','#FCBE72'), showInLegend = T)|>
  hc_yAxis(
