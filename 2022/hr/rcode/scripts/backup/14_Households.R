HouseholdsSourceTable <- as.data.frame(HouseholdsSourceTable)
# ED Table
...
HouseholdsBinded <- bind_rows(HouseholdsED,HouseholdsAC)%>%filter(Type.of.Occupancy!="Total")
HouseholdsBinded$Region <- factor(HouseholdsBinded$Region, levels = c(ED,AC))
...
HouseholdsPlot2 <- highchart()|>       
  hc_add_series(HouseholdsBinded, "column", hcaes(x = Type.of.Occupancy2, y = PercentageOfPopulation, color = colouract,  group = Region), color = c('#405381','#FCBE72'), showInLegend = T)|>
  hc_yAxis(
