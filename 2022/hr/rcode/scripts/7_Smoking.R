SmokingWider  <- as.data.frame(SmokingWider)
...
SmokingEDACState <- rbind(SmokingED, SmokingAC)%>%dplyr::select(-c(Census.Year, Statistic))
SmokingEDACState$Region <- factor(SmokingEDACState$Region, levels = c(ED,AC))
...
SmokingPlot2 <- highchart()|>       
  hc_add_series(SmokingEDACState, "column", hcaes(x = Census.Year, y = PercentageWhoSmokeTobaccoProducts, color = colouract,  group = Region), color = c('#405381','#FCBE72'), showInLegend = T)|>
  hc_yAxis(
