...
EduAllLessT <- bind_rows(EduACLessT,EduEDLessT,EduStateLessT)
EduAllLessT$Region <- factor(EduAllLessT$Region, levels = c(ED,AC,"State"))
#plot
...
EduPlot2 <- highchart()|>       
  hc_add_series(EduAllLessT, "column", hcaes(x = Highest.Level.of.Education.Completed2, y = PercentageOfPopulation, color = colouract,  group = Region), color = c('#405381','#FCBE72','#13C1A5'), showInLegend = T)|>
  hc_yAxis(
