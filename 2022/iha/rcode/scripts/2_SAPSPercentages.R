...
SAPSTotalRegion <- SAPSTotalWHR%>%select(-c(GEOGID,GUID,GEOGDESC,IHA))%>%group_by(Region)%>%dplyr::summarise(across(everything(), sum,na.rm=T))
SAPSTotalRegion$RegionType <- "HR"								  
SAPSTotalIHA <- SAPSTotalWHR%>%select(-c(GEOGID,GUID,GEOGDESC,Region))%>%group_by(IHA)%>%dplyr::summarise(across(everything(), sum,na.rm=T))%>%dplyr::rename("Region" = "IHA")
SAPSTotalIHA$RegionType <- "IHA"								
SAPSTotal <- rbind(SAPSTotalIHA,SAPSTotalRegion)#facto
