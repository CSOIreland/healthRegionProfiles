...
VolunteersBinded$Region <- factor(VolunteersBinded$Region, levels = c(ED,AC,"State"))
# Plot
...
VolunteerPlot2 <- highchart()|>       
  hc_add_series(VolunteersBinded, "column", hcaes(x = Region, y = PercentageOfPopulation, color = colouract,  group = Region), color = c('#405381','#FCBE72','#13C1A5'), showInLegend = T)|>
  hc_yAxis(
...
VolunteersState <- as.data.frame(VolunteersState)
