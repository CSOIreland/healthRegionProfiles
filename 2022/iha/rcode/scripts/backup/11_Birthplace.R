...
BirthplaceOIAC <- sum(BirthAC$value[BirthAC$Location != AC &BirthAC$Location != "Not stated" &BirthAC$Location != "Total"])
...
BirthLessTAll$Region <- factor(BirthLessTAll$Region, levels = c(ED,AC,"State"))
#plot
...
BirthPlot2 <- highchart()|>       
  hc_add_series(BirthLessTAll, "column", hcaes(x = Location2, y = PercentageOfPopulation, color = colouract,  group = Region), color = c('#405381','#FCBE72','#13C1A5'), showInLegend = T)|>
  hc_yAxis(
