...
RenewableEnergyBinded$Region <- factor(RenewableEnergyBinded$Region, levels = c(ED,AC,"State"))
...
RenewableEnergyPlot2 <- highchart()|>       
  hc_add_series(RenewableEnergyBinded, "column", hcaes(x = RenewableEnergy2, y = PercentageOfPopulation, color = colouract,  group = Region), color = c('#405381','#FCBE72','#13C1A5'), showInLegend = T)|>
  hc_yAxis(
