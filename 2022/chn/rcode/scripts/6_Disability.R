...
# IHA subtabels
IHADisWiderAllSexes <- DisWider%>%filter(Region == IHA & Sex == "Both Sexes")
IHADisWiderAllSexes$PercentageWithADisability <- IHADisWiderAllSexes$value*100/TotalPopIHABothSexes
IHADisWiderMale <- DisWider%>%filter(Region == IHA & Sex == "Males")
IHADisWiderMale$PercentageWithADisability <- IHADisWiderMale$value*100/TotalPopIHAMales
IHADisWiderFemale <- DisWider%>%filter(Region == IHA & Sex == "Females")

IHADisWiderFemale$PercentageWithADisability <- IHADisWiderFemale$value*100/TotalPopIHAFemales
...
DisBinded <- bind_rows(EDDisWiderAllSexes,IHADisWiderAllSexes,StateDisWiderAllSexes,ACDisWiderAllSexes)
DisBinded$Region <- factor(DisBinded$Region, levels = c(ED,IHA,AC,"State"))
#plot disability
...
DisBinded$colouract[DisBinded$Region == IHA] <- '#90989f'
DisPlot2 <- highchart()|>       
  hc_add_series(DisBinded, "column", hcaes(x = Region, y = PercentageWithADisability, color = colouract,  group = Region), color = c('#405381','#FCBE72','#13C1A5','#90989f'), showInLegend = T)|>
  hc_yAxis(
...
IHADisWiderFemale$PercentageWithADisability <- sprintf("%.1f", round(IHADisWiderFemale$PercentageWithADisability,1)) 

IHADisWiderMale$PercentageWithADisability <- sprintf("%.1f", round(IHADisWiderMale$PercentageWithADisability,1)) 