...
#ac table and percentages
PESIHA <- PESSource%>%filter(Region == IHA)
PESIHA$PercentageOfPopulation <- PESIHA$value*100/PESIHA$value[PESIHA$Principle.Economic.Status == "Total"]

#IHA individual metrics
IHAPESTot <- PESIHA$value[PESIHA$Sex == "Both Sexes" & PESIHA$Principle.Economic.Status == "Total"]
IHAUnemployed <- sum(PESIHA$value[PESIHA$Sex == "Both Sexes" & (PESIHA$Principle.Economic.Status == "Short term unemployed" |PESIHA$Principle.Economic.Status == "Long term unemployed")])
IHAUnemployedPerc <- sprintf("%.1f", round(IHAUnemployed*100/IHAPESTot,1))
IHAStudent <- PESIHA$value[PESIHA$Sex == "Both Sexes" & PESIHA$Principle.Economic.Status == "Student"]
IHAStudentPerc <- sprintf("%.1f", round(IHAStudent*100/IHAPESTot,1))
IHAUnableToWork <- PESIHA$value[PESIHA$Sex == "Both Sexes" & PESIHA$Principle.Economic.Status == "Unable to work due to permanent sickness or disability"]
IHAUnableToWorkPerc <- sprintf("%.1f", round(IHAUnableToWork*100/IHAPESTot,1))
...
PESIHALessT <- PESIHA%>%filter(Principle.Economic.Status!= "Total")


#wrap labels and factorise
PESIHALessT$Principle.Economic.Status2 <- stringr::str_wrap(PESIHALessT$Principle.Economic.Status, 25)
PESIHALessT$Principle.Economic.Status2 <- factor(PESIHALessT$Principle.Economic.Status2, levels = c("At work", "Looking for first regular\njob","Short term unemployed","Long term unemployed", "Student", "Looking after home/family","Retired","Unable to work due to\npermanent sickness or\ndisability","Other"))
...
PESLessTAll$colouract[PESLessTAll$Region == AC] <- '#FCBE72'
PESLessTAll$colouract[PESLessTAll$Region == IHA] <-'#90989f'

PESPlot2 <- highchart()|>       
  hc_add_series(PESLessTAll, "column", hcaes(x = Principle.Economic.Status2, y = PercentageOfPopulation, color = colouract,  group = Region), color = c('#405381','#FCBE72','#13C1A5','#90989f'), showInLegend = T)|>
  hc_yAxis(
...
PESIHA$PercentageOfPopulation <- sprintf("%.1f", round(PESIHA$PercentageOfPopulation,1))
