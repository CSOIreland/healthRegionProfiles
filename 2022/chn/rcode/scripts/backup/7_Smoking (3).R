...
SmokingIHA <- SmokingWider%>%filter(Region == IHA)

#Bind three tables for plot
SmokingEDACState <- rbind(SmokingED, SmokingAC,SmokingIHA,SmokingState)%>%dplyr::select(-c(Census.Year, Statistic))
SmokingEDACState$Region <- factor(SmokingEDACState$Region, levels = c(ED,IHA,AC,"State"))
...
SmokingEDACState$colouract[SmokingEDACState$Region == IHA] <- '#90989f'

SmokingEDACState <- as.data.frame(SmokingEDACState)
SmokingPlot2 <- highchart()|>       
  hc_add_series(SmokingEDACState, "column", hcaes(x = Census.Year, y = PercentageWhoSmokeTobaccoProducts, color = colouract,  group = Region), color = c('#405381','#FCBE72','#13C1A5','#90989f'), showInLegend = T)|>
  hc_yAxis(
...
SmokingIHA$PercentageWhoSmokeTobaccoProducts <- sprintf("%.1f", round(SmokingIHA$PercentageWhoSmokeTobaccoProducts,1))
