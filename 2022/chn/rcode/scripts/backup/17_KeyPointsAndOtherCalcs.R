...
IHAAged0to14 <- PopSourceTableRegrouped$value[PopSourceTableRegrouped$Region == IHA & PopSourceTableRegrouped$Sex == "Both Sexes" & PopSourceTableRegrouped$Age == "0-14"]
IHAAged65P <- PopSourceTableRegrouped$value[PopSourceTableRegrouped$Region == IHA  & PopSourceTableRegrouped$Sex == "Both Sexes" & PopSourceTableRegrouped$Age == "65 years and over"]
IHAAged1564 <- sum(PopSourceTableRegrouped$value[PopSourceTableRegrouped$Region == IHA  & PopSourceTableRegrouped$Sex == "Both Sexes" & (PopSourceTableRegrouped$Age == "15-24" |PopSourceTableRegrouped$Age == "25-44" |PopSourceTableRegrouped$Age == "45-64" )])
IHAAgeTotal <- PopSourceTableRegrouped$value[PopSourceTableRegrouped$Region == IHA  & PopSourceTableRegrouped$Sex == "Both Sexes" & PopSourceTableRegrouped$Age == "Total"]
IHAAgeDependency <- round((IHAAged0to14+IHAAged65P)*100/IHAAged1564,1)
IHAYouthDependency <- round((IHAAged0to14)*100/IHAAged1564,1)
IHAOldAgeDependency <- round((IHAAged65P)*100/IHAAged1564,1)

IHAAged0to14Perc <- sprintf("%.1f", round(IHAAged0to14*100/TotalPopIHABothSexes,1))
IHAAged15to64Perc <- sprintf("%.1f", round(IHAAged1564*100/TotalPopIHABothSexes,1))
IHAAged65PPerc <- sprintf("%.1f", round(IHAAged65P*100/TotalPopIHABothSexes,1))
