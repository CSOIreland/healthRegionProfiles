...
# IHA table
CarersIHA <- CarersSourceTable%>%filter(Region == IHA)

#Total pop added so perentage of population can be calculated
CarersIHA$TotalPop <- 0
CarersIHA$TotalPop[CarersIHA$Sex == "Males"] <- TotalPopIHAMales
CarersIHA$TotalPop[CarersIHA$Sex == "Females"] <- TotalPopIHAFemales
CarersIHA$TotalPop[CarersIHA$Sex == "Both Sexes"] <- TotalPopIHABothSexes

# Calculate percentage of pop
CarersIHA$PercentageOfPopulation <- CarersIHA$value*100/CarersIHA$TotalPop

# Bind all tables for plot
CarersBinded <- bind_rows(CarersED,CarersIHA,CarersAC, CarersState)
CarersBinded$Region <- factor(CarersBinded$Region, levels = c(ED,IHA,AC,"State"))
# Plot
...
CarersBinded$colouract[CarersBinded$Region == IHA] <- '#90989f'
CarePlot2 <- highchart()|>       
  hc_add_series(CarersBinded, "column", hcaes(x = Sex, y = PercentageOfPopulation, color = colouract,  group = Region), color = c('#405381','#FCBE72','#13C1A5','#90989f'), showInLegend = T)|>
  hc_yAxis(
...
CarersIHA$PercentageOfPopulation <- sprintf("%.1f", round(CarersIHA$PercentageOfPopulation,1))
