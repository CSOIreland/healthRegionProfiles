...
# Pop tables for IHA
PopIHAMales <- PopSourceTableRegrouped%>%filter(Region == IHA & Sex == "Males")
PopIHAFemales <- PopSourceTableRegrouped%>%filter(Region == IHA & Sex == "Females")
PopIHABothSexes<- PopSourceTableRegrouped%>%filter(Region == IHA & Sex == "Both Sexes")

# Males IHA with percentage
PopIHAMalesFull <- PopSourceTable%>%filter(Region == IHA & Sex == "Males")
PopIHAMalesFull$Percentage <-PopIHAMalesFull$value*100/PopIHAMalesFull$value[PopIHAMalesFull$Age == "Total"]

#Males IHA without total (for plots)
PopIHAMalesFullLessT <- PopIHAMalesFull%>%filter(Age!="Total")
PopIHAMalesFullLessT$Gender <- "Male"
PopIHAMalesFullLessT$Age <- factor(PopIHAMalesFullLessT$Age, levels = c("Age 0-4","Age 5-9","Age 10-14","Age 15-19","Age 20-24","Age 25-29","Age 30-34","Age 35-39","Age 40-44","Age 45-49","Age 50-54","Age 55-59","Age 60-64","Age 65-69","Age 70-74","Age 75-79","Age 80-84","Age 85 and over"))

#Females IHA full table with percentage
PopIHAFemalesFull <- PopSourceTable%>%filter(Region == IHA & Sex == "Females")
PopIHAFemalesFull$Percentage <- PopIHAFemalesFull$value*100/PopIHAFemalesFull$value[PopIHAFemalesFull$Age == "Total"]

#Females IHA without total
PopIHAFemalesFullLessT <- PopIHAFemalesFull%>%filter(Age!="Total")
PopIHAFemalesFullLessT$Gender = "Female"
PopIHAFemalesFullLessT$Age <- factor(PopIHAFemalesFullLessT$Age, levels = c("Age 0-4","Age 5-9","Age 10-14","Age 15-19","Age 20-24","Age 25-29","Age 30-34","Age 35-39","Age 40-44","Age 45-49","Age 50-54","Age 55-59","Age 60-64","Age 65-69","Age 70-74","Age 75-79","Age 80-84","Age 85 and over"))


#Both sexes IHA with percentage
PopIHABothSexesFull<- PopSourceTable%>%filter(Region == IHA & Sex == "Both Sexes")
PopIHABothSexesFull$Percentage <- sprintf("%.1f", round(PopIHABothSexesFull$value*100/PopIHABothSexesFull$value[PopIHABothSexesFull$Age == "Total"],1))

#both sexes IHA wthout total (for plots)
PopIHABothSexesFullLessT <- PopIHABothSexesFull%>%filter(Age!="Total")
PopIHABothSexesFullLessT$Age <- factor(PopIHABothSexesFullLessT$Age, levels = c("Age 0-4","Age 5-9","Age 10-14","Age 15-19","Age 20-24","Age 25-29","Age 30-34","Age 35-39","Age 40-44","Age 45-49","Age 50-54","Age 55-59","Age 60-64","Age 65-69","Age 70-74","Age 75-79","Age 80-84","Age 85 and over"))


# Total population of males, females and both sexes in IHA
TotalPopIHAFemales <- PopIHAFemalesFull$value[PopIHAFemalesFull$Age == "Total"]
TotalPopIHAMales <- PopIHAMalesFull$value[PopIHAMalesFull$Age == "Total"]
TotalPopIHABothSexes <- PopIHABothSexesFull$value[PopIHABothSexesFull$Age == "Total"]
...
PopIHAMalesFullLessT$Percentage <- sprintf("%.1f", round(as.numeric(PopIHAMalesFullLessT$Percentage)*-1,1))
PopIHAFemalesFullLessT$Percentage <- sprintf("%.1f", round(PopIHAFemalesFullLessT$Percentage,1))
PopIHABothSexesFullLessT$Percentage <- sprintf("%.1f", round(as.numeric(PopIHABothSexesFullLessT$Percentage),1))
